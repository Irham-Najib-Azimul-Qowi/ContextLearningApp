  ...
  return (
    <TeacherWorkspaceShell activeGroupId="questions">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {/* Generator Form */}
        <form onSubmit={handleGenerate} className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-xs space-y-5 text-xs">
          {/* Subject */}
          <div>
            <label className="font-bold text-slate-900 block mb-2">Mata Pelajaran (Fokus SD)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="font-bold text-slate-900 block mb-1.5">Tingkat Kelas</label>
              <select
                value={grade}
                onChange={(e) => setGrade(Number(e.target.value))}
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                {[1, 2, 3, 4, 5, 6].map((g) => (
            ...
            <div>
              <label className="font-bold text-slate-900 block mb-1.5">Bentuk Soal</label>
              <select
                value={type}
          ...
          {/* Topic */}
          <div>
            <label className="font-bold text-slate-900 block mb-1.5">Materi / Topik Pembelajaran</label>
            <input
              type="text"
              required
              value={topic}
              ...
              className="w-full px-4 py-2.5 rounded-xl border border-slate-200 font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          ...
          <button
            ...
            className="w-full py-3.5 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-sm shadow-sm transition-transform active:scale-98 flex items-center justify-center gap-2"
          >
            {isGenerating ? (
              <>
