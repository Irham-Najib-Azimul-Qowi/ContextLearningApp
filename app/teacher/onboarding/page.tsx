          ...
          {step === "search_school" && (
            <div className="space-y-5">
                      ...
                      >
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-xs text-foreground">{school.name}</span>
                            <span className="text-[11px] text-secondary-text">
                              · {school.educational_level === "SD" ? "Sekolah Dasar" : school.educational_level === "SMP" ? "SMP" : "SMA"}
                            </span>
                          </div>
              ...
              {selectedExistingSchool && (
                <form onSubmit={handleJoinSchoolSubmit} className="space-y-3 pt-2 border-t border-border">
                  <div>
                  ...
                  {joinSuccessMsg && (
                    <div className="p-2.5 rounded-lg bg-success-subtle text-success text-xs font-medium flex items-center gap-2 border border-emerald-200">
          ...
          {step === "create_school" && (
            <form onSubmit={handleCreateSchoolSubmit} className="space-y-4">
              <div className="flex items-center justify-between border-b border-border pb-3">
                <div>
                  <h2 className="text-base font-bold text-foreground">Daftarkan Sekolah Baru</h2>
                  <p className="text-xs text-secondary-text">Isi identitas dan wilayah geografis sekolah</p>
                </div>
                <Button variant="ghost" size="sm" onClick={() => setStep("choice")} className="text-xs">
                  <ArrowLeft className="w-3.5 h-3.5 mr-1" /> Kembali
                </Button>
              ...
              {/* Accessible Segmented Control for Educational Level */}
              <div>
                <label className="text-xs font-semibold text-foreground block mb-1.5">
                  Jenjang Pendidikan Instansi:
                </label>
                <div
                  className="inline-flex w-full p-1 bg-[#F2F4F8] border border-border rounded-lg gap-1"
                  role="radiogroup"
                ...
                >
                  {(["SD", "SMP", "SMA"] as EducationLevel[]).map((lvl) => {
                    ...
                    const labelDesc =
                      lvl === "SD" ? "Sekolah Dasar (1–6)" : lvl === "SMP" ? "SMP (7–9)" : "SMA/SMK (10–12)";
                    return (
                      <button
                        key={lvl}
                        type="button"
                        ...
                        className={`flex-1 py-2 px-2.5 rounded-md text-xs font-semibold transition-all cursor-pointer text-center ${
                          isSel
                            ? "bg-surface text-primary shadow-xs border border-border"
                            : "text-secondary-text hover:text-foreground hover:bg-white/50"
                        }`}
                      >
                        <span>{lvl}</span>
                        <span className="hidden sm:inline text-[11px] font-normal text-secondary-text ml-1.5">
                          · {labelDesc}
                        </span>
                      </button>
                    );
